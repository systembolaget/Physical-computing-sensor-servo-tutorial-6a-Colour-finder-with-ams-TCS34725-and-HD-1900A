...
// the servo and interface with the sensor; use the latest version
#include <Wire.h>
#include "Servo.h"
...
// "training"; matching servo rotation angles for 10° colour wheel
...
const byte timeInterval = 7; // Duration until the next servo rotation
...
void setup()
{
  ...
  // Uncomment for colour "training" and checking
  //Serial.begin(19200);
...
void loop()
{
  ...
  readSensor();
  // Iterates through the array to retrieve matching colour sample
  identifySample();
  rotateServo();
}
...
void identifySample()
{
  ...
  for (byte i = 0; i < samplesCount; i++)
  {
    ...
    // and when using the sensor, the lower the colour distance maximum
    ...
    {
      // Retrieve the matching angle value (in microseconds) from array
      angleTarget = SAMPLES[i][3];
      ...
      /*Serial.print("Distance: "); Serial.println(colourDistance);
        ...
        Serial.print("Microseconds: "); Serial.println(angleTarget);*/
    }
  }
}
...
void rotateServo() 
{
  ...
  if (millis() - timeNow >= timeInterval)
  {
    ...
    timeNow = millis();
    // Don't write to the servo after target angle was reached
...
{
  // getRawData() would cause a delay, the duration of the selected
  // integration time, thus blocking loop() like a delay() in the code,
  // but using an ISR (= interrupt) method, there is no more delay,
